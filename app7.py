...
def apply_excel_format5(writer, sheet_name, df):
    """
    Applique une mise en forme automatique à une feuille Excel.
    ...
    # Définir les styles
    header_format = workbook.add_format({'bold': True, 'align': 'center', 'bg_color': '#D7E4BC', 'border': 1})
    cell_format = workbook.add_format({'border': 1})

    # Appliquer la mise en forme aux en-têtes
    for col_num, value in enumerate(df.columns.values):
        worksheet.write(0, col_num, value, header_format)
...
def export_excel5(duplicate_dict, combined_duplicates, df_original, original_without_duplicates):
    ...
    # Remplacer les NaN et INF par une valeur par défaut (par exemple, 0 ou une chaîne vide)
